...
DECLARE	@discipline			AS	SMALLINT=(SELECT discipline_id		FROM Disciplines	WHERE discipline_name LIKE(N'???????????%C++'));
...
DECLARE @teacher			AS	INT		=(SELECT teacher_id FROM Teachers WHERE first_name LIKE(N'????'));
